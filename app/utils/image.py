...
def _oss_signature_generator(callback, resource):
    ...
    return date, base64.encodebytes(hmac.new(current_app.config['OSS_ACCESS_SECRET'].encode(),
...
def oss_authorization(item_id, filename):
    ...
    date, signature = _oss_signature_generator(callback_base64, resource_path)
    headers = {'Authorization': 'OSS %s:%s' % (current_app.config['OSS_ACCESS_ID'], signature),
               'x-oss-callback': callback_base64, 'Content-Type': 'image/jpeg', 'Date': date}
